...
import type { TrackAnalysis } from "@/lib/analyze-track";
      ...
      <div className="flex items-start gap-3 sm:gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-3 sm:mt-4 h-12 sm:h-16 rounded-lg bg-muted/30 relative overflow-hidden">
            <div className="absolute inset-0 flex items-end justify-center gap-[1px] sm:gap-[2px] px-1 sm:px-2">
              {track.waveform.map((amplitude, index) => (
                <div
                  key={`${track.id}-wave-${index}`}
                  className="flex-1 bg-primary/40 rounded-full transition-colors duration-300 hover:bg-primary"
                  style={{
                    height: `${Math.max(10, amplitude * 100)}%`,
                    animation: `pulse-glow ${2 + (index % 5) * 0.3}s ease-in-out infinite`,
